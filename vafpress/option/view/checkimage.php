<tr class="vp-checkimage" <?php echo VP_Util_Text::print_if_exists($validation, 'data-vp-validation="%s"'); ?> id="<?php echo $name; ?>" id="<?php echo $name; ?>">
	...
	<td class="fields">
		...
		<label>
			...
			<img src="<?php echo $item->img; ?>" alt="<?php echo $item->label; ?>" class="vp-js-tipsy" style="<?php VP_Util_Text::print_if_exists($item_max_width, 'max-width: %spx; '); ?><?php VP_Util_Text::print_if_exists($item_max_height, 'max-height: %spx; '); ?>" original-title="<?php echo $item->label; ?>" />
		</label>
		<?php endforeach; ?>
